...
NTD_IP = ["172.16.26.3", "172.16.26.4", "172.16.26.9", "172.17.26.10", "172.16.26.12", "172.16.26.7", "172.16.26.15", "172.17.26.16", "172.17.26.17"]
...
def sync_ntd(server, hosts):
    ...
    header = '\x55\xaa\x00\x00\x01\x01\x00\xc1\x00\x00\x00\x00\x00\x00\x0f\x00\x00\x00\x0f\x00\x10\x00\x00\x00\x00\x00\x00\x00'
    footer = '\x00\x00\x0d\x0a'

    year1 = chr(ntp_date.year/256)
    year2 = chr(ntp_date.year%256)
    
    data = header + year2 + year1 + chr(ntp_date.month)+ chr(ntp_date.day)+ chr(ntp_date.hour)+ chr(ntp_date.minute)+ chr(ntp_date.second) + footer

    
    for host in hosts:
        open_ntd_count += 1
        thread.start_new_thread( send_time, (host, data, server, ) )
...
def start():
    ...
    ntp_server_name = raw_input("Server: ")

    sync_time = 60 * input("Schedule Synchronization time (in minutes): ")

    bias = input("Bias (in Seconds): ")
    call = NTPClient()
